...
class STrack(BaseTrack):
    ...
    def update_features(self, reid_feat, diffusion_feat, alpha=0.95):
        self.curr_feat = reid_feat
        self.diffusion_feat = diffusion_feat
        # alpha = 0.8 if self.virtual_frames > 0 else alpha 
        self.diffusion_embs = alpha * self.diffusion_embs + (1 - alpha) * diffusion_feat
        self.diffusion_embs /= np.linalg.norm(self.diffusion_embs)
        
        
        self.emb = alpha * self.emb + (1 - alpha) * reid_feat
    ...
    def re_activate_mamba(self, new_track, frame_id, new_id=False, state = None):
        ...
        if new_track.curr_feat is not None:
            self.update_features(new_track.curr_feat, new_track.diffusion_feat)
        ###########################################
    ...
    def update_mamba(self, new_track, frame_id, update_feature = False, state = None, score = None):
        ...
        if state == "Virtual":
            ...
            if update_feature:
                self.update_features(new_track.curr_feat, new_track.diffusion_feat , alpha = 0.85)
    ...
    def update(self, output_results, img_info, img_size, img, tag):
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                ...
                track.update_features( det.emb, det.diffusion_embs, alp)
        ...
        for itracked, idet in matches:
            ...
            unconfirmed[itracked].update_features(detections[idet].emb, detections[idet].diffusion_embs, alp)
